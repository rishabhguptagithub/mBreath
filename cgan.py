...
inp = pd.read_csv('Data/Input/inputSubject16resp.csv', sep = ',', header = None)
out = pd.read_csv('Data/Input/outputSubject16resp.csv', sep = ',', header = None)

mb_size = 64
Z_dim = 100
X_dim = np.shape(inp)
y_dim = np.shape(np.array(out))
#print(X_dim,y_dim)
h_dim = 128
y_dim = y_dim[1]
X_dim = X_dim[1]
...
for it in range(1000000):
    ...
    X_mb, y_mb = next_batch(mb_size, inp, out)
    #X_mb = inp.iloc[:mb_size, :]
    #y_mb = out.iloc[:mb_size,:]
    """
    i = 0
    while (mb_size * (i+1)<len(inp)):
        X_mb.append(inp.iloc[i*mb_size:(i+1)*mb_size,:])
        y_mb.append(out.iloc[i*mb_size:(i+1)*mb_size,:])
    X_mb.append(inp.iloc[i*mb_size:,:])
    y_mb.append(out.iloc[i*mb_size:,:])
    """
